...
| **Agent (swarm)** | URL ± source | `xevon agent swarm -t URL [--source ./app]` | AI plans modules + extensions, native scanner runs them, optional triage loop |
| **Agent (autopilot)** | URL ± source | `xevon agent autopilot -t URL [--source ./app]` | One long LLM session driving the scan autonomously |
...
├── No
│   ├── Quick single-URL test? ──────────── xevon scan-url <URL>
│   ├── Want fast results? ──────────────── xevon scan -t URL --strategy lite
│   ├── Standard scan? ─────────────────── xevon scan -t URL
│   ├── Maximum external recon? ─────────── xevon scan -t URL --strategy deep
│   └── Custom extension scripts only? ──── xevon run extension -t URL --ext script.js
│
└── Yes
    ├── Have source code only (no live target)?
    │   ├── One-shot code review? ───────── xevon agent query --prompt-template security-code-review --source ./app
    │   └── Multi-phase AI audit? ────────── xevon agent audit --source ./app
    │
    └── Have a target URL?
        ├── AI directs the native scanner ── xevon agent swarm -t URL [--source ./app]
        └── AI is the scanner ───────────── xevon agent autopilot -t URL [--source ./app]
```
